...
Citizen.CreateThread(function()
    local suitcase = {Config.Prop}
        ...
        exports['qtarget']:AddTargetModel(suitcase, {
            options = {
                {
                    event = "sg-workbench:pickUpBag", 
                    icon = "fas fa-hand-paper",
                    label = Config.PickupBTN,
                },
                {
                    event = "sg-workbench:emsMedBag",
                    icon = "fas fa-hand-paper",
...
RegisterNetEvent('sg-workbench:pickUpBag',function()
	...
	TriggerServerEvent('sg-workbench:getItemForEMS', Config.Item)
    TriggerServerEvent("sg-workbench:deleteBag", ObjToNet(closestObject))
end)

RegisterNetEvent('sg-workbench:deleteBag', function(netId)
    if DoesEntityExist(NetToObj(netId)) then
        DeleteObject(NetToObj(netId))
    end   
end)

RegisterNetEvent('sg-workbench:useMedBag',function()
...
RegisterNetEvent('sg-workbench:emsMedBag')
AddEventHandler('sg-workbench:emsMedBag',function()
